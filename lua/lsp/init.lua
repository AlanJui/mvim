...
local function setup_lsp_client_environment()
    ...
    ---------------------------------------------------------------------------
    -- `mason-lspconfig` provides extra, opt-in, functionality that allows you
    -- to automatically set up LSP servers installed via `mason.nvim` without
    -- having to manually add each server setup to your Neovim configuration.
    -- It also makes it possible to use newly installed servers without having
    -- to restart Neovim!
    ---------------------------------------------------------------------------
    local LSP_SERVERS = {
        "lua_ls",
    ...
    ---------------------------------------------------------------------------
    -- 2023-04-09: 由於目前尚無法百分之百確認，mason_lspconfig 與
    -- mason_tool_installer 分工的界限；且兩者皆有自動安裝 LSP Client 的功能，
    -- 恐有作業時會相互影響 之問題發生，故暫先停用 mason_tool_installer 插件。
    ---------------------------------------------------------------------------
    --
